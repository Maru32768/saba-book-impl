...
impl HtmlTokenizer {
    ...
    fn set_self_closing_flag(&mut self) {
        ...
        if let Some(t) = self.latest_token.as_mut() {
            match t {
                HtmlToken::StartTag { ref mut self_closing, .. } => *self_closing = true,
                _ => panic!("`latest_token` should be either StartTag")
            }
        }
    }
...
mod tests {
    ...
    fn test_attributes() {
        ...
        "class".chars().for_each(|x| attr1.add_char(x, true));
        attr1.add_char('A', false);

        let mut attr2 = Attribute::new();
        "id".chars().for_each(|x| attr2.add_char(x, true));
        attr2.add_char('B', false);

        let mut attr3 = Attribute::new();
        "foo".chars().for_each(|x| attr3.add_char(x, true));
        "bar".chars().for_each(|x| attr3.add_char(x, false));
